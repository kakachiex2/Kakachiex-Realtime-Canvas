...
export function useComfy(): UseComfyReturn {
  ...
  const [comfyHost, setComfyHost] = useLocalStorage<string>(
    "comfy_host_url",
    "http://127.0.0.1:8000",
  );
  ...
  // Heartbeat to detect if backend dies (since Vite proxy keeps WS alive artificially)
  useEffect(() => {
    let interval: NodeJS.Timeout;
    if (status === "CONNECTED") {
      interval = setInterval(async () => {
        try {
          const res = await fetch("/comfyui-api/system_stats");
          if (!res.ok) {
            disconnect();
          }
        } catch (e) {
  ...
  const connect = useCallback(
    async (hostUrl: string) => {
      ...
      try {
        ...
        if (!statsRes.ok) {
          throw new Error(
            `Failed to connect to ComfyUI. Ensure it is running at http://127.0.0.1:8000.`,
          );
        }
        ...
        try {
          ...
          if (!stats || !stats.system) {
            // ComfyUI system_stats has a 'system' object
            throw new Error(
              "Connected to server but it doesn't look like ComfyUI (invalid response).",
            );
          }
        } catch (e) {
          throw new Error(
            "Connected to server but failed to parse response. Is it ComfyUI?",
          );
        }
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (msg.type === "executed" && msg.data?.output?.images) {
              ...
              setStatus("ERROR");
              setError(
                `Workflow failed: ${msg.data?.exception_type || "Unknown error"}`,
              );
            }
          } catch (e) {
            // ignore binary
  ...
  const runWorkflow = useCallback(
    async (
      ...
      // 3. Heuristic Updates
      // Update LoadImage
      const imageNodes = Object.values(workflow).filter(
        (n: any) => n.class_type === "LoadImage",
      );
      ...
      for (const [key, node] of Object.entries(workflow)) {
        ...
        if (
          ...
          n.class_type === "SamplerCustom" ||
          n.class_type === "SamplerCustomAdvanced"
        ) {
          if (n.inputs.noise_seed !== undefined) {
            n.inputs.noise_seed = Math.floor(Math.random() * 1000000000);
          } else if (n.inputs.seed !== undefined) {
            n.inputs.seed = Math.floor(Math.random() * 1000000000);
          }
        }

        // Fix Windows colon Error 22
        if (n.class_type === "SaveImage") {
          n.inputs.filename_prefix = "Kakachiex/sketch";
        }
      }

      // 4. Queue
      console.error(
        "--- COMFYUI PAYLOAD DUMP ---",
      ...
      try {
        fetch("/api/debug", {
          ...
          console.error("Debug interceptor failed", e);
        });
      } catch (e) {
      ...
      if (!queueResp.ok) {
        ...
        throw new Error(
          `Failed to queue prompt: ${queueResp.status} ${errText}`,
        );
      }
    },
